...
@pytest.mark.regression
def test_regression_optimize_ab_winner_golden(example_gear):
    """Locked AB optimize outcome for the user's gear / 27k / reset 155 scenario."""
    result = optimize(
        OptimizeConfig(
            target_hp=27000,
            int_reset_level=155,
...
def test_regression_policy_labels_stable():
    assert policy_short(PolicyName.MP_WASH_SHORTFALL) == "A"
    assert policy_short(PolicyName.INT_DUMP_SHORTFALL) == "B"
    assert policy_short(PolicyName.MP_WASH_HARDCORE) == "C"
    assert policy_short(PolicyName.INT_ONLY_PLAIN) == "D"
